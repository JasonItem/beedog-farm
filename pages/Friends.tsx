...
import { User, InventoryItem, ITEMS } from '../types.ts';
import { Store, Coins, X, Plus, Minus, ShoppingBag } from 'lucide-react';
...
const Friends: React.FC<MarketProps> = ({ user, inventory, onBuy, onSell }) => {
    ...
    // Filter Buyable items (Seeds)
    const buyableItems = Object.values(ITEMS).filter(item => item.type === 'seed');
    ...
    const handleConfirmSell = () => {
        if (selectedSellItem) {
            ...
            // Note: We do NOT reset mode here, so user stays on Sell tab
        }
    };
        ...
        <div className="bg-[#FFF8E7] min-h-screen pb-32 animate-fade-in flex flex-col relative">
            <div className="pt-12 px-6 pb-6 bg-white rounded-b-[40px] shadow-sm z-10 sticky top-0">
                ...
                <div className="flex bg-gray-100 p-1 rounded-full relative">
                    ...
                    <button onClick={() => setMode('buy')} className={`flex-1 relative z-10 font-bold text-sm py-2.5 transition-colors ${mode === 'buy' ? 'text-brand-brown' : 'text-gray-400'}`}>
                        买种子
                    </button>
                    <button onClick={() => setMode('sell')} className={`flex-1 relative z-10 font-bold text-sm py-2.5 transition-colors ${mode === 'sell' ? 'text-brand-brown' : 'text-gray-400'}`}>
                        卖货物
                    </button>
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-[150] flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white w-full max-w-xs rounded-[32px] p-6 shadow-2xl animate-pop-in relative">
                        ...
                        <div className="flex items-center justify-between bg-gray-50 rounded-2xl p-2 mb-6 border border-gray-100">
                             ...
                             <button 
                                onClick={() => setSellQuantity(Math.min(selectedSellItem.max, sellQuantity + 1))}
                                className="w-10 h-10 bg-white rounded-xl shadow-sm flex items-center justify-center text-brand-brown active:scale-90 transition-transform"
                             >
                        ...
                        <div className="flex justify-center mb-6">
                            <button onClick={() => setSellQuantity(selectedSellItem.max)} className="text-xs font-bold text-brand-orange bg-brand-orangeLight px-3 py-1 rounded-lg active:scale-95">
