...
import { CloudSun, Droplets, Shovel, Moon, Coins, Plus, Minus, CheckCircle, Axe, Hammer, Scissors, Zap, Fish, X, MapPin, ArrowLeft } from 'lucide-react';
...
const Home: React.FC<HomeProps> = ({ user, gameState, onAction, onSleep, isVisiting, visitingName, onReturnHome }) => {
  const [selectedPlotId, setSelectedPlotId] = useState<number | null>(null);
  const [showFishingGame, setShowFishingGame] = useState(false);
  
  const seasonInfo = getSeasonInfo(user.day);

  // --- Fishing Logic (Kept same as before) ---
  const startFishing = () => {
      if (isVisiting) {
            ...
            <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col items-center gap-4 pointer-events-auto">
                {!isVisiting && (
                    <>
                        ...
                        <button 
                            onClick={onSleep} 
                            className="bg-brand-brown/90 backdrop-blur-md text-white w-12 py-5 rounded-3xl font-black shadow-lg flex flex-col items-center justify-center gap-2 active:scale-90 transition-all border-b-4 border-black/20 hover:bg-brand-brown"
                            title="休息/存档"
                        >
        ...
        {selectedPlot && !isVisiting && (
            <div className="absolute bottom-28 left-4 right-4 bg-white/85 backdrop-blur-xl rounded-[32px] p-6 shadow-2xl z-[110] animate-slide-up border border-white/60 flex flex-col gap-4 ring-1 ring-black/5 pointer-events-auto">
                ...
                <div className="flex gap-3 overflow-x-auto no-scrollbar py-2">
                    ...
                    {/* Empty State for Sand and others with no actions */}
                    ...
                    {selectedPlot.type === 'soil' && (
                        <>
                            {/* 浇水按钮：如果是已种植且非枯萎状态，始终显示。如果已浇水或成熟则禁用 */}
